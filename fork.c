...
static inline void free_task_struct(struct task_struct* tsk)
{
     ...
     */
            ...
            /*
             * vmalloc() is a bit slow, and calling vfree() enough times will
             * force a TLB flush.  Try to minimize the number of calls by
             * caching stacks.
             */
...
static unsigned long* alloc_thread_stack_node(struct task_struct* tsk, int node)
{
     ...
     */
    stack = __vmalloc_node_range(THREAD_SIZE,
                                 THREAD_ALIGN,
                                 VMALLOC_START,
                                 VMALLOC_END,
...
static inline void free_thread_stack(struct task_struct* tsk)
{
    ...
    if (vm)
    {
        ...
        for (i = 0; i < NR_CACHED_STACKS; i++)
        {
            if (this_cpu_cmpxchg(cached_stacks[i], NULL, tsk->stack_vm_area) !=
                NULL)
                continue;
...
void thread_stack_cache_init(void)
{
    thread_stack_cache = kmem_cache_create_usercopy(
        "thread_stack", THREAD_SIZE, THREAD_SIZE, 0, 0, THREAD_SIZE, NULL);
...
static void account_kernel_stack(struct task_struct* tsk, int account)
{
    ...
    if (vm)
        mod_lruvec_page_state(
            vm->pages[0], NR_KERNEL_STACK_KB, account * (THREAD_SIZE / 1024));
    else
        mod_lruvec_slab_state(
            stack, NR_KERNEL_STACK_KB, account * (THREAD_SIZE / 1024));
}
...
#ifdef CONFIG_VMAP_STACK
    cpuhp_setup_state(
        CPUHP_BP_PREPARE_DYN, "fork:vm_stack_cache", NULL, free_vm_stack_cache);
#endif
    ...
    struct pid* pid, int trace, int node, struct kernel_clone_args* args)
{
    ...
    /* CMPT 432/832 CHANGE BY dns682, wde364, 2025, 02, 02*/
    /*first need to kmalloc a kfifo queue then initialize queue and semaphores*/
    p->minfo.front = 0;
    p->minfo.back  = 0;
    p->minfo.size  = 0;
    p->minfo.buf   = kmalloc(128 * sizeof(pid_t), GFP_KERNEL);
    if (!p->minfo.buf)
    {
        goto bad_fork_free;
    }
    spin_lock_init(&p->minfo.mutex);
    spin_lock_init(&p->minfo.destroy_mutex);
    ...
    {
        pid = alloc_pid(
            p->nsproxy->pid_ns_for_children, args->set_tid, args->set_tid_size);
        if (IS_ERR(pid))
        {
            retval = PTR_ERR(pid);
...
struct mm_struct* copy_init_mm(void) { return dup_mm(NULL, &init_mm); }
...
struct task_struct* create_io_thread(int (*fn)(void*), void* arg, int node)
{
    ...
    struct kernel_clone_args args = {
        .flags =
            ((lower_32_bits(flags) | CLONE_VM | CLONE_UNTRACED) & ~CSIGNAL),
                                              ...
                                              size_t usize)
{
    ...
    *kargs = (struct kernel_clone_args) {
    ...
    if (args.set_tid && copy_from_user(kset_tid,
...
void walk_process_tree(struct task_struct* top,
...
int sysctl_max_threads(struct ctl_table* table,
                       int write,
                       void* buffer,
                       size_t* lenp,
                       loff_t* ppos)
